...
(async () => {
  ...
  try {
    ...
    const checks = await page.evaluate(async () => {
      ...
      check('Prepared duplicate background restoration also keeps its exact size and position', !cutout.removeBg && sameRect(cutoutRect, getAddedLayerRect(cutout))); removeBgButton.click();
      check('Prepared duplicate background removal keeps its exact size and position', cutout.removeBg && sameRect(cutoutRect, getAddedLayerRect(cutout)));
