...
- The resulting product is stored as a transparent in-memory canvas with placement bounds mapped to source pixel coordinates. Shared cleanup trims broad silhouette edges by one source pixel, then smooths binary silhouette coverage with a separable 5 × 5 binomial filter. Coverage is remapped to an inward opacity ramp, giving broad straight edges approximately 96/223/255 alpha instead of the old abrupt 191/255 transition. Source RGB stays untouched and source alpha is multiplied only at the silhouette, retaining texture and internal translucent regions. A two-pixel-deep core test protects one-to-four-pixel features and their connections from the wider filter. Cleanup uses actual transparent neighbors rather than the outside of the canvas and cannot spread opacity back into removed background. This also applies to manual Remove BG. The canvas dimensions and pre-trim placement bounds stay unchanged, preserving fitted size and center.
...
`tests/cutout-smoothing.browser.cjs` passes 22 checks covering gradual edge opacity, curved/slanted silhouettes, one-to-four-pixel details and connections, unchanged internal color/translucency, original-resolution large-image preparation, fixed geometry, repeated removal, Undo/Redo and PNG/JPG/WebP export. The optional `CUTOUT_PREVIEW` PNG sheet was inspected over light/dark backgrounds at native and magnified sizes. Checks use generated fixtures; the reported issue did not include a source photo. Existing subject-preservation and cutout/scale suites continue to pass (23 and 31 checks).
