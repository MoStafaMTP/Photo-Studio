...
Image exports keep the uploaded filename stem with the selected JPG/PNG/WebP extension, without the former `-photo-studio` suffix. Conflicting names in a ZIP receive a numeric suffix to prevent replacement during extraction.

**DT/PT unmain outputs:** the Listing review plans one additional Normal-template image for each resolved DT or PT `main` source. Apply Workflow creates these copies as independent, editable left-side images with ` unmain` appended before the extension; the originals retain their current Main/Passenger template selection. Copies keep explicit `unmain` metadata and a `generatedFromImageId` link to their source, independent image URLs and layer assets. Reapplying does not create extra copies, and a matching uploaded unmain file is reused. Deleting a generated copy allows one replacement on the next run. Descriptive filenames, existing filename collisions and repeated source filenames are handled. Other roles, including cv/io/numbered/unmain, shared components and Full Set (`DPTB`), do not create these DT/PT copies. Missing Normal templates block the workflow, with the existing single-template account fallback retained.
...
`tests/listing-unmain.browser.cjs` adds 26 checks for the DT/PT output workflow: reviewed pending copies, actual Apply Workflow and downloads, independent edits/assets, idempotence and recreation after deletion, landscape centers, metadata precedence and round trips, existing unmain reuse, load/missing-template failures, filename collisions, and DPTB exclusion. The metadata suite has eight tests including descriptive trailing-unmain filenames and the Full Set code.
