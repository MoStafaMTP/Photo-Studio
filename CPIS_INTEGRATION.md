...
DSA eBay uses `DSA Seat Factory - eBay` for every image category regardless of material, unless CPIS supplies a valid explicit `templateName`. Its material question is hidden in Listing, and local uploads can apply the workflow without selecting a material. The CPIS payload contract still requires material and preserves the supplied value; a local DSA plan without CPIS metadata reports `material: null` rather than inventing a material.
...
For accounts other than DSA eBay, Apply Workflow creates a local Normal-template `unmain` output for each resolved DT/DB `main` image, preserving the original image and its template. DSA does not generate these copies, even when CPIS supplies DT/DB main roles. Existing supplied or previously generated images remain in the batch. The new filename appends ` unmain` before the extension. CPIS metadata, not filename substrings, controls this rule; PT and DPTB (Full Set) do not generate these copies. Copies appear in the editor and exports, with their own editor IDs, `unmain` subtype and `generatedFromImageId` source links in the plan. Existing generated copies or matching uploaded unmain files are reused. Photo Studio does not invent CPIS IDs or change composite/listing membership. Previously created PT unmain images remain available for editing or manual removal.
...
Individual and batch image filenames retain their stems without a Photo Studio suffix. A batch assigned entirely to DSA eBay exports images at the ZIP root with no directories. Other or mixed-account batches contain `Main/` and `unmain/`, including empty directories when necessary. Images assigned Normal templates go in `unmain/`; all other images go in `Main/`. Routing uses the actual current template, not the browsed account, image subtype or filename, so an Other Material main image using Normal also goes in `unmain/`. ZIP filename collisions are numbered within each folder or at the flat ZIP root so no images overwrite one another on extraction. The archive itself keeps its account-folder name.
...
Standalone filenames containing `Full Set`, `Full_Set`, `Full-Set`, or `FullSet` are classified as `DPTB`. Main images use the account/material's existing Main template; supporting suffixes still select Normal. This does not introduce a template named DPTB or rename uploaded files. CPIS continues to send the canonical variation code `DPTB`.
