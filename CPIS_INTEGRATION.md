...
Individual and batch image filenames retain their stems without a Photo Studio suffix. Each ZIP contains `Main/` and `unmain/`, including empty directories when necessary. Images assigned Normal templates go in `unmain/`; all other images go in `Main/`. Routing uses the actual current template, not the image's subtype or filename, so an Other Material main image using Normal also goes in `unmain/`. ZIP filename collisions are numbered within each folder so no images overwrite one another on extraction. The archive itself keeps its account-folder name.
